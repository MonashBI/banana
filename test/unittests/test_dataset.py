...
class TestDicomTagMatch(BaseTestCase):
    ...
    DICOM_MATCH = [
        InputFileset('gre_phase', dicom_format, GRE_PATTERN,
                     dicom_tags={IMAGE_TYPE_TAG: PHASE_IMAGE_TYPE},
                     is_regex=True),
        InputFileset('gre_mag', dicom_format, GRE_PATTERN,
    ...
    def test_order_match(self):
        study = self.create_study(
            TestMatchStudy, 'test_dicom',
            inputs=[
                InputFileset('gre_phase', dicom_format,
                             pattern=self.GRE_PATTERN, order=1,
                             is_regex=True),
                InputFileset('gre_mag', dicom_format,

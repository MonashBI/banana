...
class ConversionStudy(Study, metaclass=StudyMetaClass):
    ...
    def conv_pipeline(self):
        ...
        pipeline.connect_output('nifti_gz_from_dicom', nifti_gz_from_dicom, 'file')
        ...
        pipeline.connect_output('mrtrix_from_nifti_gz', mrtrix_from_nifti_gz, 'file')
        ...
        pipeline.connect_output('directory_from_zip', directory_from_zip, 'file')
        # Convert from NIfTI.gz to MRtrix format on output
        zip_from_directory = pipeline.add(
            'zip_from_directory',
            IdentityInterface(fields=['file']))
        pipeline.connect_input('directory', zip_from_directory, 'file')
        pipeline.connect_output('zip_from_directory', zip_from_directory, 'file')
        return pipeline

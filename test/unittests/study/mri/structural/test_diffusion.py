...
class TestDiffusion(BaseTestCase):
    ...
    def test_preprocess(self):
        study = self.create_study(
            DwiStudy, 'preprocess', [
                InputFileset('magnitude', 'r_l_dwi_b700_30', mrtrix_image_format),
                InputFileset('dwi_reference', 'l_r_dwi_b0_6', mrtrix_image_format)])
    ...
    def test_extract_b0(self):
        study = self.create_study(
            DwiStudy, 'extract_b0', [
                InputFileset('preproc', 'preproc', nifti_gz_format),
                InputFileset('grad_dirs', 'gradient_dirs', fsl_bvecs_format),
                InputFileset('bvalues', 'bvalues', fsl_bvals_format)])
    ...
    def test_tensor(self):
        study = self.create_study(
            DwiStudy, 'tensor', [
                InputFileset('bias_correct', 'bias_correct', nifti_gz_format),
                InputFileset('brain_mask', 'brain_mask', nifti_gz_format),
                InputFileset('grad_dirs', 'gradient_dirs', fsl_bvecs_format),
                InputFileset('bvalues', 'bvalues', fsl_bvals_format)])
    ...
    def test_fod(self):
        study = self.create_study(
            DwiStudy, 'fod', [
                ...
                InputFileset('brain_mask', 'brain_mask', nifti_gz_format),
                InputFileset('grad_dirs', 'gradient_dirs', fsl_bvecs_format),
                InputFileset('response', 'response', text_format),
                InputFileset('bvalues', 'bvalues', fsl_bvals_format)])
    ...
    def test_intensity_normalization(self):
        study = self.create_study(
            DwiStudy, 'intens_norm', [
                InputFileset('bias_correct', 'biascorrect', nifti_gz_format),
                InputFileset('brain_mask', 'brainmask', nifti_gz_format),
                InputFileset('grad_dirs', 'gradientdirs', fsl_bvecs_format),
                InputFileset('bvalues', 'bvalues', fsl_bvals_format)])
    ...
    def test_concatenate(self):
        study = self.create_study(
            NODDIStudy, 'concatenate', inputs=[
                InputFileset('low_b_dw_scan', 'r_l_dwi_b700_30', mrtrix_image_format),
                InputFileset('high_b_dw_scan', 'r_l_dwi_b2000_60', mrtrix_image_format)])
...
#                 InputFileset('preproc', 'noddi_dwi', mrtrix_image_format),
#                 InputFileset('brain_mask', 'roi_mask', analyze_format),
#                 'grad_dirs': Fileset('noddi_gradient_directions',
#                                      fsl_bvecs_format),
#                 InputFileset('bvalues', 'noddi_bvalues', fsl_bvals_format)})

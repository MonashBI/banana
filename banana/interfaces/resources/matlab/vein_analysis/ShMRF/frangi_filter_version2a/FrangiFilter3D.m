...
defaultoptions = struct('FrangiScaleRange', [1 8], 'FrangiScaleRatio', 2, 'FrangiAlpha', 0.5, 'FrangiBeta', 0.5, 'FrangiC', 500, 'verbose',true,'BlackWhite',true);
...
for i = 1:length(sigmas),
    ...
    if(i==1)
        ...
        if(nargout>2)
            ...
            %Voutx=expRa; Vouty=expRb; Voutz=expS;
        end
    else
        if(nargout>1)
        ...
        if(nargout>2)
            ...
            %Voutx(Voxel_data>Iout)=expRa(Voxel_data>Iout);
            %Vouty(Voxel_data>Iout)=expRb(Voxel_data>Iout);
            %Voutz(Voxel_data>Iout)=expS(Voxel_data>Iout);
        end

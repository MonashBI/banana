...
class MeanDisplacementCalculationOutputSpec(TraitedSpec):
    ...
    mean_displacement_rc = File(exists=True, desc='mean displacement values '
                                ...
                                'millisecond. If, for example, a analysis lasts '
...
class MeanDisplacementCalculation(BaseInterface):
    ...
    def _run_interface(self, runtime):
        ...
        analysis_start_time = list_inputs[0][1]
        list_inputs = [
        ...
        analysis_len = int((list_inputs[-1][1] + float(list_inputs[-1][2])) *
                        1000)
        mean_displacement_rc = np.zeros(analysis_len) - 1
        motion_par_rc = np.zeros((6, analysis_len)) - 1
        mean_displacement = []
        motion_par = []
        idt_mat = np.eye(4)
        all_mats = []
        ...
        for f in list_inputs:
            ...
            if len(mats) > 1:  # for 4D files
                for i, mat in enumerate(mats):
                    ...
                    start_times.append((
                        dt.datetime.strptime(
                            str(analysis_start_time), '%H%M%S.%f') +
                ...
                start_times.append((
                    dt.datetime.strptime(str(analysis_start_time),
                                         '%H%M%S.%f') +
                ...
                motion_par_rc[:, int(start_scan * 1000):
                              int(end_scan * 1000)] = np.array(
                                  [mp, ] * duration).T
        start_times.append((
            dt.datetime.strptime(str(analysis_start_time), '%H%M%S.%f') +
...
class CreateMocoSeriesInputSpec(BaseInterfaceInputSpec):
    ...
    start_times = File(exists=True, mandatory=True, desc='start times of all '
                       'the sequences (or volumes) acquired in the analysis ('
...
class FixedBinningInputSpec(BaseInterfaceInputSpec):
    ...
    start_times = File(desc='Start times of all the scans in the analysis. This '

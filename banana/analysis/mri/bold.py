...
def create_multi_fmri_class(name, t1, epis, epi_number, echo_spacing,
    ...
    subcomp_specs = [SubCompSpec('t1', T1wAnalysis)]
    ...
    subcomp_specs.append(SubCompSpec('epi_0', BoldAnalysis, epi_refspec))
    if epi_number > 1:
        epi_refspec.update({'t1_wm_seg': 'coreg_ref_wmseg',
                            't1_preproc': 'coreg_ref',
                            'train_data': 'train_data',
        ...
        subcomp_specs.extend(SubCompSpec('epi_{}'.format(i), BoldAnalysis,
                                        epi_refspec)
...
#         for i in range(epi_number))
    ...
    dct['add_subcomp_specs'] = subcomp_specs

...
class DwiAnalysis(EpiSeriesAnalysis, metaclass=AnalysisMetaClass):
    ...
    add_data_specs = [
        ...
        FilesetSpec('eddy_qc', directory_format, 'eddy_qc_pipeline',
                    desc=("QC output generated by Eddy preprocessing tool")),
        FilesetSpec('eddy_qc_summary', json_format, 'eddy_qc_pipeline',
                    desc=("Study-wise database containing quality metrics and "
                          "data info."), frequency='per_dataset'),
        FilesetSpec('eddy_qc_report', pdf_format, 'eddy_qc_pipeline',
                    desc=("Study-wise database containing quality metrics and "
                          "data info."), frequency='per_dataset'),
    ...
    def eddy_qc_pipeline(self, **name_maps):

        pipeline = self.new_pipeline(
            name='eddy_qc',
            name_maps=name_maps,
            desc=(
                "Run QC on the Eddy preprocessing step"))

        eddy_quad = pipeline.add(
            'eddy_quad',
            EddyQuad(),
            inputs={
                'idx_file': (),
                'param_file': ('eddy_par', eddy_par_format),
    ...
    def eddy_qc_group_pipeline(self, **name_maps):

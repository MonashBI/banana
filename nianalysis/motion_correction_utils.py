...
import glob
import shutil
from xnatutils import ls as xnat_ls
import errno
...
def local_motion_detection(input_dir, pet_dir=None, pet_recon=None,
    ...
    if dcm:
        ...
        if struct2align is not None:
            shutil.copyfile(struct2align, working_dir)
...
def inputs_generation(scan_description, input_dir, siemens=False):
    ...
    epis = raw_input("Please select the EPI scans: ").split()
    dwi = raw_input("Please select the DWI scans (including both b0 and"
    ...
    t2s = raw_input("Please select the all the other scans the do not belog to"
